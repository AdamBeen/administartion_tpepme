...
from security_testing import list_security_cases, run_security_test, run_security_test_from_uploads, get_report_path, screen_uploaded_payload
...
async def analyze(
    ...
    security_mode: str = Form("unsecured"),
    ...
    security_result = screen_uploaded_payload(security_mode, question, uploaded_files)
    if security_result.get("blocked"):
        ...
        return JSONResponse(
            ...
            content={
                ...
                "cleaned_response": {
                    "recommandation": {
                        "decision_proposee": "escalade",
                        "niveau_confiance": 1.0,
                        "justification": "Le dossier contient des signaux de manipulation ou d'instruction non fiable.",
                    },
                    "hitl": {
                        "requis": True,
                        "niveau": "ROUGE",
                        "raison": "Contenu bloque par le controle de securite.",
                        "validateur_recommande": "Validation manuelle",
                    },
                    "reponse_proposee": "Analyse bloquee: le contenu transmis contient des instructions non fiables ou des signaux de manipulation.",
                },
                "final_json": {
...
async def security_cases():
    return {"cases": list_security_cases()}


@app.post("/api/security/run")
async def security_run(
    mode: str = Form("unsecured"),
    question: str = Form(""),
    files: list[UploadFile] = File(default=[]),
):
    try:
        ...
        for f in files:
            content = await f.read()
            uploaded_files.append({"filename": f.filename, "content": content, "size": len(content)})
        return run_security_test_from_uploads(mode=mode, question=question, uploaded_files=uploaded_files)
    except Exception as e:
        logger.error("[/api/security/run] error: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/security/report/{report_id}/json")
async def security_report_json(report_id: str):
    path = get_report_path(report_id, "json")
    if not path:
        raise HTTPException(status_code=404, detail="Report not found")
    return FileResponse(str(path), media_type="application/json", filename=f"{report_id}.json")


@app.get("/api/security/report/{report_id}/markdown")
async def security_report_markdown(report_id: str):
    path = get_report_path(report_id, "md")
    if not path:
        raise HTTPException(status_code=404, detail="Report not found")
    return FileResponse(str(path), media_type="text/markdown", filename=f"{report_id}.md")
